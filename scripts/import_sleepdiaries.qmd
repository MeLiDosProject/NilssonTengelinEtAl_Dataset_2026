...
```{r}
#if sleepdelay is 300 or higher, it is assumed that this person meant seconds instead of minutes

data <- 
data |> 
  rename(wake = offset, sleepprep = sleep) |> 
  mutate(sleepdelay = replace_when(sleepdelay, sleepdelay >= 300 ~ sleepdelay/60),
         sleep = sleepprep + dminutes(sleepdelay),
...
## Estimate missing sleep entries

```{r}
avg_sleep <- 
data |> 
  summarize(.by = record_id,
    sleep_duration2 = mean(sleep_duration, na.rm = TRUE)
  )

data <- 
data |> 
  left_join(avg_sleep, by = "record_id") |> 
  mutate(sleep_duration = replace_values(sleep_duration, NA ~ sleep_duration2),
         sleep = wake - sleep_duration,
         sleepprep = sleep - dminutes(sleepdelay)) |> 
  select(-sleep_duration2)
